...
The second line `FROM log_in_attempts` specifies the table to return these columns from.  
The last line, `WHERE login_time > ‘18:00:00’ AND success = 0;`, filters to return only the login time after 7pm that had a failed login attempt. 
...
This is returning all columns in the `log_in_attempts` database. The `WHERE` line is specified to only return data on the two dates, 2022-05-09 and 2022-05-08 by using the `OR` statement.
