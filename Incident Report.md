...
| **Justification** | In this instance, restricting access at a user level, even if the data was shared, would still restrict the user as they would not be authorized to access the shared data. <br>The implementation of a temporary access process would allow for temporary access to data that would be removed once the user exits the data, or after a certain amount of time. <br>Regular audits and logs of activity would allow for more unauthorized accesses to be caught. Whether that is through a discovery of personnel accessing data they shouldn’t on the logs through a regular audit or an activity log alert. |
